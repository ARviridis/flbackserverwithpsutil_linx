...
def timer_thread():
    while True:
        ...
        mi4 = psutil.disk_io_counters(perdisk=True , nowrap = True)['PhysicalDrive4']
        ...
        cpu_percent.append([mi3.user,mi3.system,mi3.idle])
        ...
        if (rr1 > 0) and (ttrr1 <= 0):
            time_disk.append([(round(rr1 / 1024)) / 1024 / cfg.time_step, (write)])
        if (rr1 <= 0) and (ttrr1 > 0):
            time_disk.append([read, (round(ttrr1 / 1024)) / 1024 / cfg.time_step])
        if (rr1 <= 0) and (ttrr1 <= 0):
            time_disk.append([read, write])
        if (rr1 > 0) and (ttrr1 > 0):
            time_disk.append([(round(rr1 / 1024)) / 1024 / cfg.time_step, (round(ttrr1 / 1024) / 1024 )/ cfg.time_step])
        if (read < 0): read = 0
        else:read = (round(rr1 / 1024)) / 1024 / cfg.time_step
        if (write < 0): write = 0
        else:write = (round(ttrr1 / 1024)) / 1024 / cfg.time_step
        ...
        if (ccc1+wcc1)>0:time_disk1.append(cfg.time_step/round(ccc1+wcc1))
