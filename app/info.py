...
def get_mem_info(blocks):
    ...
    fig= plt.figure(figsize=(3 * cfg.fig_hw, cfg.fig_hw))#2*4
...
def get_disks_usage(blocks):
                ...
                for dp in psutil.disk_partitions():
                    fig = plt.figure(figsize=(3 * cfg.fig_hw, cfg.fig_hw))
                    fig.set_facecolor('#F1F1F2')
                    plt.subplot(121)
                    try:
                ...
                plt.grid(True, which='both')
                plt.ylim(0)
                fig.savefig('static/stickers_proxy5.png', facecolor=fig.get_facecolor(), edgecolor='none')
                f = ('static/stickers_proxy5.png')
                blocks.append({
...
def time_disk(blocks):
    ...
    fig = plt.figure(figsize=(3* cfg.fig_hw, cfg.fig_hw))  # 2*4
    fig.set_facecolor('#F1F1F2')
    time_disk = psutil.disk_io_counters(perdisk = False , nowrap = False )
    lines = list()
    lines.append(str.format('read_time: %s' % round(time_disk.read_time/1000,3) + 's'))
    lines.append(str.format('write_time: %s' % round(time_disk.write_time/1000,3) + 's'))
